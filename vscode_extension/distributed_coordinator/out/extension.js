...
class DistributedCoordinatorProvider {
    constructor(context) {
        this.context = context;
        this._onDidChangeTreeData = new vscode.EventEmitter();
        this.onDidChangeTreeData = this._onDidChangeTreeData.event;
        this.nodes = [];
        this.coordinatorStatus = null;
    ...
    getChildren(element) {
        if (!element) {
            ...
            if (this.coordinatorStatus) {
                items.push(new NodeItem(`协调器状态: ${this.coordinatorStatus.status}`, `活跃节点: ${this.coordinatorStatus.activeNodes}/${this.coordinatorStatus.totalNodes}`, vscode.TreeItemCollapsibleState.None, 'coordinator'));
            }
            // 添加节点
            this.nodes.forEach(node => {
                const statusIcon = node.status === 'online' ? '🟢' :
                    node.status === 'busy' ? '🟡' : '🔴';
                items.push(new NodeItem(`${statusIcon} ${node.nodeId}`, `CPU: ${node.cpuUsage}% | 内存: ${node.memoryUsage}% | 任务: ${node.activeTasks}`, vscode.TreeItemCollapsibleState.None, 'node'));
            });
    ...
    async startStatusPolling() {
        const refreshInterval = vscode.workspace.getConfiguration('powerautomation.distributed').get('refreshInterval', 5000);
        setInterval(async () => {
            try {
...
class PerformanceWebviewProvider {
...
PerformanceWebviewProvider.viewType = 'powerautomation.performance';
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('powerautomation.performance', performanceProvider));
    // 注册命令
    const commands = [
        ...
        vscode.commands.registerCommand('powerautomation.distributed.showStatus', async () => {
            const panel = vscode.window.createWebviewPanel('coordinatorStatus', '协调器状态', vscode.ViewColumn.One, { enableScripts: true });
            panel.webview.html = `
                <html>
                <body>
                    <h1>🎯 分布式协调器状态</h1>
    ...
    // 自动启动
    const autoStart = vscode.workspace.getConfiguration('powerautomation.distributed').get('autoStart', true);
    if (autoStart) {
