                ...
                for mcp_name, mcp_info in self.registered_mcps.items():
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            # 修复：直接处理MCP响应，不期望success字段
                            if mcp_data.get('status') == 'running':
                                aggregated_status["components"][mcp_name] = {
                                    "status": "运行中",
                                    "description": mcp_info.get('description', mcp_data.get('name', mcp_name)),
                                    "version": mcp_data.get('version', 'unknown'),
                                    "capabilities": mcp_data.get('capabilities', []),
                                    "performance": mcp_data.get('performance', {}),
                                    "port": mcp_info['port']
                                }
                                ...
                                aggregated_status["components"][mcp_name] = {
                                    "status": "错误",
                                    "error": f"MCP状态: {mcp_data.get('status', 'unknown')}",
