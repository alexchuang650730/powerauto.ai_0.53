#!/usr/bin/env python3
"""
PowerAutomation Level 1 單元測試 - kilocode_adapter
...
class TestKilocodeadapterAsync(unittest.IsolatedAsyncioTestCase):
    """
    kilocode_adapter 異步單元測試類
...
def run_tests():
    """運行所有測試"""
    # 同步測試
    sync_suite = unittest.TestLoader().loadTestsFromTestCase(TestKilocodeadapter)
    sync_runner = unittest.TextTestRunner(verbosity=2)
    sync_result = sync_runner.run(sync_suite)
    
    # 異步測試
    async_suite = unittest.TestLoader().loadTestsFromTestCase(TestKilocodeadapterAsync)
...
if __name__ == '__main__':
    success = run_tests()
    if success:
        print(f"✅ kilocode_adapter_unit 單元測試全部通過!")
    else:
        print(f"❌ kilocode_adapter_unit 單元測試存在失敗")
        sys.exit(1)
