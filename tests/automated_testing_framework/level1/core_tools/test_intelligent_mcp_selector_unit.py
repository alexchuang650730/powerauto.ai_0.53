#!/usr/bin/env python3
"""
PowerAutomation Level 1 單元測試 - intelligent_mcp_selector
...
class TestIntelligentmcpselector(unittest.TestCase):
    """
    intelligent_mcp_selector 單元測試類
...
class TestIntelligentmcpselectorAsync(unittest.IsolatedAsyncioTestCase):
    """
    intelligent_mcp_selector 異步單元測試類
...
def run_tests():
    """運行所有測試"""
    # 同步測試
    sync_suite = unittest.TestLoader().loadTestsFromTestCase(TestIntelligentmcpselector)
    sync_runner = unittest.TextTestRunner(verbosity=2)
    sync_result = sync_runner.run(sync_suite)
    
    # 異步測試
    async_suite = unittest.TestLoader().loadTestsFromTestCase(TestIntelligentmcpselectorAsync)
