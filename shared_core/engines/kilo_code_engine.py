...
class KiloCodeEngine:
    ...
    async def start(self, host: str = '0.0.0.0', port: int = 8765) -> bool:
        ...
        try:
            ...
            logger.info(f"Kilo Code引擎啟動成功: {host}:{port}")
            return True
            
        except Exception as e:
            logger.error(f"Kilo Code引擎啟動失敗: {e}")
            self.is_running = False
            return False
    ...
    async def _handle_connection(self, websocket, path):
        """處理WebSocket連接"""
        try:
            logger.info(f"新連接: {websocket.remote_address}")
            ...
            logger.info(f"連接關閉: {websocket.remote_address}")
    ...
    async def _process_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if message_type == 'intervention_request':
                return await self.intervention_coordinator.process_intervention_request(message)
                
            elif message_type == 'workflow_recording':
                return await self._handle_workflow_recording(message)
    ...
    async def _handle_workflow_recording(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """處理工作流錄製消息"""
        action = message.get('action', '')
        
        if action == 'start':
            workflow_name = message.get('workflow_name', f'工作流_{int(time.time())}')
            workflow_id = self.workflow_recorder.start_recording(workflow_name)
    ...
    async def _handle_data_processing(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """處理數據處理消息"""
        event_data = message.get('event_data', {})
        
        if not event_data:
            return {'status': 'error', 'message': '缺少事件數據'}
        ...
        event = InterventionEvent(
            ...
            intervention_needed=event_data.get('intervention_needed', False),
            confidence=event_data.get('confidence', 0.0),
            suggested_action=event_data.get('suggested_action', '')
        )
        
        result = await self.data_pipeline.process_intervention_data(event)
        return {
            'status': 'success',
            'result': result
        }
...
async def test_kilo_code_engine():
    ...
    conversation = "我不知道怎麼解決這個問題，一直出錯，幫我看看"
    event = engine.intervention_coordinator.conversation_analyzer.analyze_conversation(conversation)
    
    print(f"對話: {conversation}")
    print(f"需要介入: {event.intervention_needed}")
    print(f"信心度: {event.confidence:.2f}")
    print(f"建議動作: {event.suggested_action}")
    
    # 測試工作流錄製
    print("\n📝 測試工作流錄製")
    print("-" * 40)
    
    workflow_id = engine.workflow_recorder.start_recording("測試工作流")
    print(f"開始錄製工作流: {workflow_id}")
    
    engine.workflow_recorder.record_action("navigate", "https://example.com")
    engine.workflow_recorder.record_action("click", "#login-button")
    engine.workflow_recorder.record_action("input", "#username", "testuser")
    engine.workflow_recorder.record_action("input", "#password", "password123")
    engine.workflow_recorder.record_action("click", "#submit-button")
    
    workflow = engine.workflow_recorder.stop_recording()
    
    print(f"工作流錄製完成: {len(workflow.steps)} 個步驟")
    print(f"提取的變量: {len(workflow.variables)} 個")
    print(f"n8n節點數: {len(workflow.n8n_definition['nodes'])} 個")
    
    # 測試數據處理
    print("\n📝 測試數據處理")
    print("-" * 40)
    
    result = await engine.data_pipeline.process_intervention_data(event)
    
    print(f"RAG處理狀態: {result['rag_result']['status']}")
    print(f"RL-SRT訓練狀態: {result['training_result']['status']}")
